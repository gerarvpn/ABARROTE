...
# ============================================================
# GZMBOT - INSTALACIÓN AUTOMATIZADA (v8 - DEFINITIVA)
# - Netflix: Puppeteer Stealth + delays humanos + reintentos
# - Contactos con banderas por país
# - Panel ultraestable, cola de mensajes optimizada
# - Eliminación total de aprendizaje
# - Intervalo Netflix configurable
# ============================================================

clear
echo " ⚙️ Iniciando Instalación de GZMBOT v8 (Definitiva)"
...
# Asegurar DNS público (8.8.8.8) para evitar EAI_AGAIN
...
# ----------------------------------------------------------------------
# 3. INSTALAR NODE.JS 20 (LTS) y PM2
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 7. ARCHIVOS DE DATOS PARA NETFLIX
# ----------------------------------------------------------------------
echo '[]' > data/netflix_logs.json
echo '[]' > data/netflix_processed.json

# ----------------------------------------------------------------------
# 8. MOTOR NETFLIX (netflix-engine.js) – CON PUPPETEER STEALTH Y REINTENTOS
# ----------------------------------------------------------------------
...
class NetflixEngine extends EventEmitter {
    constructor(io) {
        ...
        this.timers = new Map();
        this.processingQueue = new Map(); // email -> boolean
    ...
    async #autoUpdateHousehold(link, accountEmail, extractedCode = null) {
        ...
        try {
            ...
            // Simular movimiento de mouse y scroll leve
    ...
    async processEmail(parsed, uid, messageId, account, messageQueue) {
        ...
        // Procesar enlace de actualización (si existe)
        if (link) {
            // Evitar procesamiento simultáneo para la misma cuenta
        ...
        // Envío del código al grupo según la opción
    ...
    async syncAccount(account, messageQueue, retries = 2) {
        let attempt = 0;
                ...
                try {
                    ...
                    for (const uid of all.slice(0, 10)) {
                        try {
                            ...
                            const logEntry = await this.processEmail(parsed, uid, mid, account, messageQueue);
                            if (logEntry) {
    ...
    async startIntervalForAccount(account, messageQueue, intervalSeconds) {
        if (this.timers.has(account.email)) {
            clearInterval(this.timers.get(account.email));
        }
        console.log(`⏱️ Iniciando intervalo para ${account.email} cada ${intervalSeconds}s`);
        const timer = setInterval(async () => {
            await this.syncAccount(account, messageQueue, 2);
        }, intervalSeconds * 1000);
        this.timers.set(account.email, timer);
        await this.syncAccount(account, messageQueue, 2);
    }
    ...
    async refreshAccounts(messageQueue) {
        ...
        const currentEmails = new Set(this.timers.keys());
        const newEmails = new Set(ncfg.accounts.map(a => a.email).filter(Boolean));
        ...
        for (let account of ncfg.accounts) {
            if (account.email && account.password && !this.timers.has(account.email)) {
                await this.startIntervalForAccount(account, messageQueue, ncfg.intervalSeconds || 120);
            }
        }
    }
...
# ----------------------------------------------------------------------
# 9. BACKEND (app.js) – OPTIMIZADO CON CONTACTOS CON BANDERAS Y ESTABILIDAD
# ----------------------------------------------------------------------
cat <<'APPEOF' > app.js
...
const countryFlags = {
    ...
    '27': '🇿🇦',  // Sudáfrica
    '234': '🇳🇬', // Nigeria
    '1809': '🇩🇴', // República Dominicana (múltiples prefijos)
    '1829': '🇩🇴',
    '1849': '🇩🇴'
};

function getFlagForNumber(number) {
    if (!number) return '🌐';
    // Extraer código de país: buscar coincidencia desde el más largo al más corto
...
function formatPhoneNumber(number) {
    ...
    // Si tiene más de 10 dígitos, formatear como +XX XXX XXX XXX
    if (digits.length > 10) {
        return '+' + digits;
    }
    return digits;
...
io.on('connection', (socket) => {
    ...
    socket.on('request_contacts', async () => {
        if (isConnected && client) {
            ...
            socket.emit('groups_update', groups);
        }
    });
    setInterval(() => {
...
async function loadContactsAndGroups() {
    ...
    try {
        ...
        contacts = rawContacts
            ...
            .map(c => {
                let rawNumber = c.number;
                // Limpiar número: eliminar todo excepto dígitos y el signo +
                let cleanNumber = rawNumber.replace(/[^0-9+]/g, '');
                if (!cleanNumber) return null;
                let formatted = formatPhoneNumber(cleanNumber);
                let flag = getFlagForNumber(cleanNumber);
                return {
                    id: c.id._serialized,
                    name: c.name || c.pushname || formatted,
                    number: formatted,
                    flag: flag,
                    rawNumber: cleanNumber
                };
            })
            .filter(c => c !== null && c.number.length >= 8);
        // Eliminar duplicados por número
        const uniqueMap = new Map();
        contacts.forEach(c => {
            if (!uniqueMap.has(c.rawNumber)) uniqueMap.set(c.rawNumber, c);
        });
...
# ----------------------------------------------------------------------
# 10. FRONTEND (index.html) – MODAL DE CONTACTOS CON BANDERAS Y ESTILO MEJORADO
# ----------------------------------------------------------------------
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .contact-item { display: flex; align-items: center; gap: 12px; padding: 12px 16px; border-radius: 16px; cursor: pointer; transition: background 0.2s; border-bottom: 1px solid #2a2a30; }
        .contact-item:hover { background: #2a2a32; }
        .contact-item.selected { background: #2563eb20; border-left: 4px solid #2563eb; }
        .contact-flag { font-size: 1.6rem; min-width: 48px; text-align: center; }
        .contact-info { flex: 1; }
        .contact-name { font-weight: 600; color: #e5e7eb; }
        .contact-number { font-size: 12px; color: #9ca3af; }
...
<body class="flex h-screen overflow-hidden">
    ...
    <div id="contactModal" class="modal" onclick="if(event.target===this) closeContactModal()">
        <div class="modal-content glass">
            <div class="flex justify-between items-center mb-5">
                <h3 class="text-2xl font-bold aluminum-title" id="modal-title">Seleccionar contacto</h3>
                <button onclick="closeContactModal()" class="text-zinc-400 hover:text-white p-2 hover:bg-white/5 rounded-full transition">
                    <i data-lucide="x" class="w-6 h-6"></i>
                </button>
            </div>
            <input type="text" id="contactSearch" placeholder="Buscar por nombre o número..." class="mb-5">
    ...
    <main id="main-content" class="flex-1 flex flex-col min-w-0 overflow-hidden" onclick="if(window.innerWidth<1024 && !document.getElementById('sidebar').classList.contains('-translate-x-full')) toggleSidebar()">
        ...
        <div class="flex-1 overflow-y-auto p-5 sm:p-7 lg:p-9 space-y-7">
            ...
            <!-- APRENDER con botón eliminar todo -->
                ...
                <div class="glass p-6 sm:p-9 rounded-3xl mb-8">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div class="relative flex items-center gap-2">
                            ...
                            <button type="button" onclick="openContactModalForField('r-phone', 'r-name')" class="p-3.5 bg-zinc-700 rounded-xl hover:bg-zinc-600 transition">
                ...
                <div class="w-full max-w-2xl mx-auto glass p-6 sm:p-9 rounded-3xl">
                    <div class="flex flex-col sm:flex-row gap-3">
                        ...
                        <div class="flex items-center gap-2 flex-1">
                            ...
                            <button type="button" onclick="openContactModalForField('e-phone', 'e-name')" class="p-3.5 bg-zinc-700 rounded-xl hover:bg-zinc-600 transition">
                                <i data-lucide="users" class="w-5 h-5"></i>
                            </button>
                        </div>
                        ...
                        <div class="mb-6">
                            ...
                            <div class="flex gap-2">
                                <div class="flex items-center gap-2 flex-1">
                                    ...
                                    <button type="button" onclick="openContactModalForField('conf-backup-phone', null)" class="p-3.5 bg-zinc-700 rounded-xl hover:bg-zinc-600 transition">
                                        <i data-lucide="users" class="w-5 h-5"></i>
                                    </button>
                                </div>
    ...
    <script>
        ...
        function openContactModalForField(phoneFieldId, nameFieldId) {
            showGroupsOnly = false;
            document.getElementById('modal-title').textContent = 'Seleccionar contacto';
            onContactSelected = (contact) => {
                if (phoneFieldId) document.getElementById(phoneFieldId).value = contact.number;
                if (nameFieldId && document.getElementById(nameFieldId)) document.getElementById(nameFieldId).value = contact.name || '';
            };
            document.getElementById('contactModal').classList.add('active');
            renderContactList();
        }

        function openGroupModalForAccount(idx) {
            showGroupsOnly = true;
            document.getElementById('modal-title').textContent = 'Seleccionar grupo';
            onContactSelected = (contact) => {
                netflixConfig.accounts[idx].groupPhone = contact.id;
        ...
        function renderContactList() {
            ...
            const source = showGroupsOnly ? groups : contacts;
            ...
            list.innerHTML = filtered.map(c => {
                const flag = c.flag ? `<span class="contact-flag">${c.flag}</span>` : '<span class="contact-flag">🌐</span>';
                return `
                    <div class="contact-item" onclick="selectContact('${c.id}', '${c.name.replace(/'/g, "\\'")}', '${c.number}')">
                        ${flag}
        ...
        function selectContact(id, name, number) {
            if (onContactSelected) {
                onContactSelected({ id, name, number });
                closeContactModal();
            }
        }
        ...
        socket.on('contacts_update', data => {
            contacts = data;
            if (document.getElementById('contactModal').classList.contains('active') && !showGroupsOnly) {
                renderContactList();
            }
        });
                ...
                learnEl.innerHTML = recentLearn.map(l => `
                    <div class="learning-item">
                        <div class="flex justify-between items-start">
                            <span class="text-xs text-zinc-500">${l.date} · ${esc(l.from)}</span>
                ...
                <div class="bg-zinc-800/50 p-4 rounded-xl space-y-3">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 items-center">
                        <div class="flex items-center gap-2">
                            <input type="text" placeholder="ID del grupo" value="${esc(acc.groupPhone||'')}" readonly class="flex-1 bg-zinc-900 cursor-pointer" onclick="openGroupModalForAccount(${idx})">
                            <button onclick="openGroupModalForAccount(${idx})" class="p-2.5 bg-zinc-700 rounded-xl hover:bg-zinc-600 transition"><i data-lucide="users" class="w-4 h-4"></i></button>
                        </div>
...
# ----------------------------------------------------------------------
# 11. INSTALAR DEPENDENCIAS DE NODE (incluyendo puppeteer-extra y stealth)
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
# 12. INSTALAR NGINX Y CONFIGURAR PROXY CON SSL
# ----------------------------------------------------------------------
...
# ----------------------------------------------------------------------
echo "===================================================="
echo "✨ GZMBOT ENTERPRISE - INSTALACIÓN COMPLETADA (v8)"
echo "===================================================="
echo "🌐 PANEL: https://$DOMAIN"
echo "👤 USUARIO: $ADMIN_USER"
echo "🔐 PASS: $ADMIN_PASS"
echo "🕐 TIMEZONE: America/Santo_Domingo (UTC-4)"
echo "⚙️ Node.js 20 LTS instalado"
echo "🚀 Respuesta instantánea (cola configurable, default 500ms)"
echo "📺 NETFLIX: solo intervalo, Puppeteer Stealth, delays humanos"
echo "🧹 Limpieza automática de aprendizaje configurable"
echo "🗑️ Botón 'Eliminar todo' funcional en Aprendizaje"
echo "🚫 Ignora notas de voz y mensajes no textuales"
echo "📞 Contactos con banderas por país (más de 30 países)"
echo "✅ Códigos Netflix: se envían al grupo si la opción está ACTIVADA"
echo "✅ Recordatorios 'Una vez' se eliminan tras enviarse"
echo "✅ Solución error EAI_AGAIN (DNS público 8.8.8.8)"
echo "===================================================="